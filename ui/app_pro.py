...
with gr.Blocks(title="Regulatory Authoring Platform") as demo:
        ...
        with gr.Tab("5 · Review"):
            ...
            with gr.Accordion("�  Multi-Reviewer Validation Package", open=True):
                ...
                with gr.Group(visible=False) as _s5_admin_panel:
                    gr.HTML(_info_box(
                        "🔐 <strong>Admin action</strong> — sections generated by the "
                        "content pipeline before the validation system existed have no "
                        "<code>publish_status.json</code>. As admin you can seed one, "
                        "attesting with your identity and a stated reason that you have "
                        "reviewed the section and consider it ready for reviewer sign-off. "
                        "This is permanently recorded in the audit trail."
                    ))
                    _s5_admin_reason = gr.Textbox(
                        label="Admin attestation reason (required; min 5 chars)",
                    ...
                    _s5_admin_seed_btn = gr.Button(
                        "🔐  Admin: Attest & seed publish status", variant="stop", scale=1,
                    )
                    _s5_admin_msg = gr.Markdown(value="")
    ...
    def _set_persona(persona):
        if not persona:
            return "", "⚠️ No persona selected.", gr.update(visible=False)
        is_admin = persona.startswith("admin@")
        status_msg = (
            f"✅ Acting as `{persona}`. All saves & approvals will be attributed to this identity."
        )
        return persona, status_msg, gr.update(visible=is_admin)
    ...
    def _admin_seed_action(selection, admin_reason, ta, dis, drug, state, iap_user):
        if not selection:
            b, c = _render_gate_cards({})
            return b, c, "⚠️ Select a section first."
        if not (iap_user or "").strip().startswith("admin@"):
            b, c = _load_gate_state(selection, ta, dis, drug, state)
            return b, c, "⛔ Only the admin persona can seed publish status."
